...
def longest_sequence(nums):
    ...
    for i in range(1, len(nums)):
        ...
        if not long_seq:
            ...
            if long_seq[1]-long_seq[0] < seq[1]-seq[0]:
                print(long_seq[1]-long_seq[0] < seq[1]-seq[0])
                long_seq = seq
    
    
    print(f'indices of longest sequence are', long_seq)
    print(f'longest sequnce is: {[nums[i] for i in range(long_seq[0], long_seq[1]+1)]}')
